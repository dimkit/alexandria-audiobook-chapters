...
def review_batch(client, model_name, batch_entries, batch_num, total_batches,
                 ...
                 min_p=0, presence_penalty=0.0, banned_tokens=None, runtime=None):
...
def main():
    ...
    if not script_document.get("entries"):
    ...
    for i, batch_info in enumerate(batches[start_batch_index:], start_batch_index + 1):
        ...
        if corrected is None:
            ...
            store.replace_script_document(
                ...
                dictionary=script_document["dictionary"],
                sanity_cache=script_document.get("sanity_cache"),
                reason="review_script_batch_failed",
                rebuild_chunks=True,
                wait=True,
            )
            save_checkpoint(
        ...
        if not passed:
            ...
            store.replace_script_document(
                ...
                dictionary=script_document["dictionary"],
                sanity_cache=script_document.get("sanity_cache"),
                reason="review_script_text_loss_guard",
                rebuild_chunks=True,
                wait=True,
            )
            save_checkpoint(
    ...
    store.replace_script_document(
        entries=all_corrected,
        dictionary=script_document["dictionary"],
        sanity_cache=script_document.get("sanity_cache"),
        reason="review_script_complete",
        rebuild_chunks=True,
        wait=True,
    )
    clear_checkpoint(CHECKPOINT_PATH)
